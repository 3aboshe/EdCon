...
const SchoolAdminDashboard: React.FC = () => {
    const { classes: classList, user, logout } = useContext(AppContext);
                    ...
                    <nav className="flex space-x-1 sm:space-x-2">
                        {(['overview', 'students', 'teachers', 'management'] as AdminTab[]).map(tab => (
                            <button
                                ...
                                className={`flex-1 px-2 sm:px-3 py-2 text-xs sm:text-sm font-medium rounded-md transition-colors ${activeTab === tab
...
const ManagementTab: React.FC<{ setSuccessMessage: (msg: string) => void }> = ({ setSuccessMessage }) => {
    ...
    const fetchCodes = async (role?: string) => {
        ...
        try {
            ...
            setCodes(userCodes as unknown as User[]);
        } catch (error) {
    ...
    const handleAddParent = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const newParent = { ...result.user, childrenIds: [] } as unknown as User;
            ...
            setNewParentName('');
            let msg = t('parent_added_with_code').replace('{code}', `<b>${result.code}</b>`);
            if (result.password) {
                msg += `<br/>Password: <b>${result.password}</b>`;
            }
    ...
    const handleAddStudent = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setUsers(prevUsers => {
                ...
                return [...updated, result.user as unknown as User];
            });

            setNewStudentName('');
            setSelectedParentForStudent(''); // Reset selection
            let msg = t('student_added_with_code').replace('{code}', `<b>${result.code}</b>`);
            if (result.password) {
                msg += `<br/>Password: <b>${result.password}</b>`;
            }
            setSuccessMessage(msg);
    ...
    const handleAddTeacher = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setNewTeacherSubject(subjects[0]?.id || '');
            setAssignedClassIds([]);
            let msg = t('teacher_added_with_code').replace('{code}', `<b>${result.code}</b>`);
            if (result.password) {
                msg += `<br/>Password: <b>${result.password}</b>`;
            }
            setSuccessMessage(msg);
                            ...
                            <tbody>
                                {codes.map(user => (
                                    <tr key={user.id} className="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td className="p-3">
                                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${user.role === 'parent' ? 'bg-blue-100 text-blue-800' :
