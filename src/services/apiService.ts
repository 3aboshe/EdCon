...
class ApiService {
  ...
  async sendGlobalNotification(data: { title: string; content: string; target: 'ALL_USERS' | 'SCHOOL_ADMINS' }): Promise<ApiResponse<any>> {
    return this.request<any>('/global-notifications', {
      method: 'POST',
      body: JSON.stringify(data)
    });
  }
